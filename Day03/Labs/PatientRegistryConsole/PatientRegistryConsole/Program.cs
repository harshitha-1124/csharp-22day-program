...
namespace PatientRegistryConsole
{
    class Program
    {
        ...
        static void UpdatePatient()
        {
            ...
            audit.Add(oldName + " changed to " + newName + " on " + DateOnly.FromDateTime(DateTime.Now));
        ...
        static void DisplayAllOldNames()
        {
            Console.WriteLine("audit list..");
            if (audit.Count == 0)
            {
                Console.WriteLine("No old patients ");
                return;
            }

            for (int i = 0; i < audit.Count; i++)
            {
                Console.WriteLine($"  {i + 1}. {audit[i]}");
            }
        }
    }
}
